...
(function($,window,undefined){
  $( window.document ).bind('mobileinit', function(){
    ...
    if ($.mobile.media("screen and (min-width:480px)")) {
    	min480 = true;
    } else if ($.mobile.browser.ie && $.mobile.browser.ie < 9) {
    	$('html').addClass('ie480');
    	// in ie 7/8 document.body is not available yet. see #39432
    	min480 = Math.max(
    	        this.documentElement["clientWidth"],
    	        this.documentElement["scrollWidth"],
    	        this.documentElement["offsetWidth"]
    	      );
    } else if ($.mobile.browser.ie) {
    	min480 = $(this).width() >= 480;
    }
...
//----------------------------------------------------------------------------------
      ...
      function getScreenHeight(){
        ...
        //GRANITE:START
        pageMin -= $(".ui-mobile-viewport[data-id=main]").offset().top;
        //GRANITE:END
      ...
      _orientationHandler = function(event){
        ...
        function popoverBtn(header) {
          if(!header.children('.popover-btn').length){
            if(header.children('a.ui-btn-left').length){
              ...
              header.children('a.ui-btn-left').replaceWith('<a class="popover-btn" data-theme="a">Menu</a>');
              //GRANITE:END
              ...
              header.prepend('<a class="popover-btn" data-theme="a">Menu</a>');
              //GRANITE:END
              header.children('a.popover-btn').addClass('ui-btn-left').buttonMarkup()
            }
          }
        ...
        function popover(){
          $menu.addClass('panel-popover')
               ...
               //GRANITE:START
//               .css({'width':'40%', 'min-width':'400px', 'display':''});
      ...
      $(window).bind('orientationchange', function(event) { setTimeout(_orientationHandler(event), 500); });
      $(window).bind('throttledresize', function(event) { setTimeout(_orientationHandler(event), 500); });
      ...
      $('.popover-btn').live('click', function(e){
        ...
        //GRANITE:START
        // this is due to header and footer is still hidden at pageshow.contentHeight event (see code at the bottom)
        var $this=$('.panel-popover'),
	        $header=$this.find(':jqmData(role="header")'),
	        $footer=$this.find(':jqmData(role="footer")');
        $this.find(':jqmData(role="content")').css({'top':$header.outerHeight(), 'bottom':$footer.outerHeight()});
        //GRANITE:END
      });
...
//----------------------------------------------------------------------------------
      ...
      $('div:jqmData(role="page")').live('pagebeforeshow.scroll', function(event, ui){
        if ($.support.touch) {
          ...
          var iscroll = $scrollArea.iscroll({});
          // GC GRANITE START
	      // resize shim https://github.com/cubiq/iscroll/issues/596
          $($scrollArea).on('scrollrefresh', function() {
        	  iscroll.refresh();
          });
          // TC GRANITE END
        }
      ...
      $('div:jqmData(role="page")').live('pageshow.contentHeight', function(){
	    ...
	    // GC GRANITE START
        $this.children(':jqmData(role="content")').trigger('scrollrefresh');
        // GC GRANITE END
      })
